...
function SeatLayout() {
  ...
  // Lấy dữ liệu ghế đã đặt theo showId cụ thể
  ...
  // Logic render ghế (disabled và màu sắc)
    ...
    <div key={row} className='flex gap-2 mt-2'>
      <div className='flex flex-wrap items-center justify-center gap-2'>
        {Array.from({length: count}, (_,i)=>{
          ...
          return (
            <button 
              // Vô hiệu hóa chức năng cho ghế đã đặt HOẶC chưa chọn giờ chiếu
              ...
              className={`flex w-10 h-10 px-3 py-3 border rounded-xl justify-center font-semibold transition duration-300 ease-in-out
                ...
                ${
                   !selectedTime // TRẠNG THÁI: Chưa chọn giờ (Vô hiệu hóa chung)
                   ? 'bg-gray-700 border-gray-700 text-white opacity-50 cursor-not-allowed'
                   : isOccupied 
                    // MÀU GHẾ ĐÃ ĐẶT 
  ...
  // Thêm useEffect để tự động chọn suất chiếu đầu tiên
  useEffect(() => {
    if (show && show.dateTime && date) {
      const availableTimes = show.dateTime[date];
      if (availableTimes && availableTimes.length > 0) {
        // Tự động chọn suất chiếu đầu tiên làm mặc định
  ...
  return show ? (
    ...
    <div className='relative flex-1 flex flex-col items-center max-md:mt-16'>
      ...
      {/* THÊM CHÚ THÍCH CÁC LOẠI GHẾ */}
      <div className='flex gap-6 mt-10 text-sm'>
          {/* Ghế Đã Đặt */}
          <div className='flex items-center gap-2'>
              <div className='w-5 h-5 bg-red-500 rounded-full opacity-50'></div>
              <p>Not Available</p>
          </div>
          {/* Ghế Đang Chọn */}
          <div className='flex items-center gap-2'>
          ...
          {/* Ghế Trống */}
          <div className='flex items-center gap-2'>
              <div className='w-5 h-5 border border-second rounded-full'></div>
              <p>Available</p>
          </div>
      </div>
